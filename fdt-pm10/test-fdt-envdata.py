...
db = pd.read_csv('pm10db_train.csv')
...
db_test = pd.read_csv('pm10db_test.csv')
...
fname = 'fuzzy_train_7334333347_trapmf_custom'
if not os.path.isfile(str(fname) + '.csv'):
    fuzz = Fuzzifier(X, y, task='reg')
    fuzz.get_clusters(init_c=[7,3,3,4,3,3,3,3,4,7])
    ...
    fdata.load_db(fname)
    # Change fvar0
    fdata.fuzzy_variables[0].setmf([('trapmf', [20,20,20,30]),
                                    ('trapmf', [20,30,40,50]),
                                    ('trapmf', [40,50,60,70]),
                                    ('trapmf', [60,70,80,90]),
                                    ('trapmf', [80,90,100,110]),
                                    ('trapmf', [100,110,120,130]),
                                    ('trapmf', [120,130,150,160])])
    # Change fvar1
    fdata.fuzzy_variables[1].setmf([('trapmf', [50,50,50,75]),
                                    ('trapmf', [50,75,125,150]),
                                    ('trapmf', [125,150,150,150])])  
    # Change fvar2
    fdata.fuzzy_variables[2].setmf([('trapmf', [50,50,50,75]),
                                    ('trapmf', [50,75,125,150]),
                                    ('trapmf', [125,150,150,150])])  
    # Change fvar3
    fdata.fuzzy_variables[3].setmf([('trapmf', [25,25,25,40]),
                                    ('trapmf', [25,35,45,60]),
                                    ('trapmf', [45,60,70,85]),
                                    ('trapmf', [70,85,85,85])])
    # Change fvar4
    ...
    # Change fvar5
    fdata.fuzzy_variables[5].setmf([('trapmf', [15,15,15,20]),
                                    ('trapmf', [15,20,25,30]),
                                    ('trapmf', [25,30,30,30])])
    # Change fvar6
...
plt.title("Timeseries of PM10 concentration (2013)")
plt.legend()
plt.show()
fig2 = plt.figure()
plt.scatter(yt, pred)
